...
namespace LMCore.TiledDungeon.Style
{
    public class StyleSelector : AbsDungeonStyle
    {
        ...
        GameObject GetWithStyle(string variant, System.Func<AbsDungeonStyle, GameObject> getter)
        {
            ...
            var style = _styles.FirstOrDefault(style => style.Provides(variant)) ?? DefaultStyle;
            Debug.Log($"StyleSelector looknig for match to: {variant}, found {style}");

            return getter(style) ?? getter(DefaultStyle);
        }
